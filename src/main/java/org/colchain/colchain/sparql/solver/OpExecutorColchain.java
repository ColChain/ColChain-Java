...
import org.colchain.colchain.sparql.iter.QueryIterColchainBlock;
...
public class OpExecutorColchain extends OpExecutor {
    private final boolean isForColchain;
    public final static OpExecutorFactory opExecFactoryColchain = new OpExecutorFactory() {
        @Override
        public OpExecutor create(ExecutionContext execCxt) {
            return new OpExecutorColchain(execCxt);
        }
    };

    protected OpExecutorColchain(ExecutionContext execCtx) {
        super(execCtx);
        isForColchain = execCtx.getActiveGraph() instanceof ColchainGraph;
    }
    ...
    protected QueryIterator execute(OpFilter opFilter, QueryIterator input) {
        ...
        if (OpBGP.isBGP(opFilter.getSubOp())) {
            ...
            ColchainGraph graph = (ColchainGraph) execCxt.getActiveGraph();
     ...
     */
    private static QueryIterator executeBGP(ColchainGraph graph, OpBGP opBGP, QueryIterator input, ExprList exprs,
                                            ExecutionContext execCxt) {
        // Execute a BGP on the real default graph
        return optimizeExecuteTriples(graph, input, opBGP.getPattern(), exprs, execCxt);
    }

    private static QueryIterator optimizeExecuteTriples(ColchainGraph graph,
    ...
    private static class OpExecutorColchainBlock extends OpExecutor {
        ...
        public QueryIterator execute(OpBGP opBGP, QueryIterator input) {
            return QueryIterColchainBlock.create(input, opBGP.getPattern(), execCxt);
        }
    }
}
